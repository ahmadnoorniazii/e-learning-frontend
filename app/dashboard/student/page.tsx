...
import { BookOpen, Clock, Award, TrendingUp, Play, Calendar, Users, Star, Target, CheckCircle } from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
            ...
            try {
              ...
              if (courseResponse.data) {
                ...
                const course: EnrolledCourse = {
                  ...
                  thumbnail: courseData.attributes.thumbnail?.data?.attributes.url || 'https://images.pexels.com/photos/11035380/pexels-photo-11035380.jpeg?auto=compress&cs=tinysrgb&w=800',
      ...
      const certificatesData: Certificate[] = certificatesResponse.data?.map(cert => ({
        id: cert.id.toString(),
  ...
  const stats = [
    ...
    {
      title: 'Hours Learned',
      value: `${Math.round(enrolledCourses.reduce((total, course) => total + (course.duration * course.progress / 100), 0) / 60)}h`,
    ...
    },
    {
      ...
      value: `${Math.round(enrolledCourses.reduce((total, course) => total + course.progress, 0) / (enrolledCourses.length || 1))}%`,
