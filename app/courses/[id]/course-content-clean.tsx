...
import { Course, Review, Progress as CourseProgress } from '@/lib/types';
...
export function CourseContent({ course, courseReviews, numericCourseId }: CourseContentProps) {
  ...
  const checkEnrollmentStatus = useCallback(async () => {
    ...
    try {
      ...
      if (enrollmentData.data && enrollmentData.data.length > 0) {
        ...
        // Set progress from enrollment data
        const enrollment = enrollmentData.data[0];
        setProgress({
          courseId: course.id,
          completedLessons: [], // Will be populated from lesson progress
  ...
  }, [user, course.id, course.lessons.length, numericCourseId]);
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      // Create enrollment using direct API call
      const enrollmentData = await makeAPICall('/enrollments', {
        method: 'POST',
        body: JSON.stringify({
          data: {
            student: user.id,
            course: courseIdForAPI,
      ...
      setError(err.message || 'Failed to enroll in this course. Please try again later.');
      
      toast({
        variant: "destructive",
        title: "Enrollment failed",
        description: err.message || "There was a problem enrolling you in this course. Please try again.",
      });
    } finally {
      setLoading(false);
    }
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {isEnrolled && progress && (
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                  ...
                  <ProgressBar value={progress.percentage} className="bg-white/20" />
